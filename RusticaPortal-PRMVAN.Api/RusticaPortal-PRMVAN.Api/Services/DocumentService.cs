...
namespace RusticaPortal_PRMVAN.Api.Services
{
    ...
    public async Task<AmountAvailableGetDTO> GetAmountAvailable(RequestInformation requestInformation, string nomSistem, string BaseDatos, string project)
    {
        ...
        public async Task<ResponseInformation> GetPeriodoSiguienteFactores(string empresa)
        {
            if (!int.TryParse(empresa, out var idEmpresa))
            {
                return new ResponseInformation
            ...
            try
            {
                ...
                cmd.Parameters.Add("@vTipo", HanaDbType.NVarChar, 30).Value = "Get_SiguientePeriodoFactores";
                cmd.Parameters.Add("@vParam1", HanaDbType.NVarChar, 50).Value = string.Empty;
                cmd.Parameters.Add("@vParam2", HanaDbType.NVarChar, 50).Value = string.Empty;
                cmd.Parameters.Add("@vParam3", HanaDbType.NVarChar, 50).Value = string.Empty;
                cmd.Parameters.Add("@vParam4", HanaDbType.NVarChar, 50).Value = string.Empty;
                ...
                if (!reader.Read())
                {
                    ...
                    {
                        Registered = false,
                        Message = "No se pudo obtener el periodo sugerido.",
                        Content = string.Empty
                    };
                }

                var periodoBase = reader["PeriodoBase"]?.ToString() ?? string.Empty;
                var periodoSiguiente = reader["PeriodoSiguiente"]?.ToString() ?? string.Empty;

                if (string.IsNullOrWhiteSpace(periodoSiguiente))
                {
                    return new ResponseInformation
                    {
                        Registered = false,
                        Message = "La tabla de factores no tiene periodos registrados.",
                        Content = string.Empty
                    };
                }
                ...
                {
                    Registered = true,
                    Message = "Periodo sugerido obtenido correctamente.",
